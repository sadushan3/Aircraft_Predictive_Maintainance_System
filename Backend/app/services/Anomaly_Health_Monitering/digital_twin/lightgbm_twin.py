"""
LightGBM Digital Twin Regressor for CA-EDT-AHMA.
...
- Trains LightGBM MultiOutputRegressor on full dev memmaps.
...
Important:
- LightGBM is not true mini-batch/incremental training.
- This file is memory-safe at the CSV-loading and prediction-output level.
- The actual LightGBM fit still happens on the full dev memmap.
"""
...
class LightGBMTwin:
    """
    Full-dev memory-safe LightGBM Digital Twin Regressor.
    """
    ...
    def __init__(
        ...
        """
        Initialize LightGBM digital twin.
        ...
        self.train_chunk_size = int(
            getattr(Config, "LGBM_TRAIN_CHUNK_SIZE", train_chunk_size)
        )
        self.prediction_chunk_size = int(
            getattr(Config, "LGBM_PREDICTION_BATCH_SIZE", prediction_chunk_size)
        )

        self.model_path: Path = getattr(
            Config,
            "LGBM_MODEL_PATH",
            Config.DIGITAL_TWIN_MODEL_DIR / "lightgbm_twin.pkl",
        )
        ...
        self.metadata_path: Path = getattr(
            Config,
            "LGBM_MODEL_METADATA_PATH",
            Config.DIGITAL_TWIN_MODEL_DIR / "lightgbm_twin_metadata.json",
        )

        self.memmap_dir: Path = getattr(
            Config,
            "LGBM_TRAIN_MEMMAP_DIR",
            Config.DIGITAL_TWIN_MODEL_DIR / "lgbm_full_dev_memmap",
        )

        self.rebuild_memmap = bool(getattr(Config, "LGBM_REBUILD_MEMMAP", True))
        self.cleanup_memmap_after_training = bool(
            getattr(Config, "LGBM_CLEANUP_MEMMAP_AFTER_TRAINING", False)
        )

        self.lgbm_train_n_jobs = int(getattr(Config, "LGBM_TRAIN_N_JOBS", 2))
        self.run_id = uuid.uuid4().hex[:12]

        self._validate_runtime_config()

        print("[PROGRESS] LightGBMTwin initialized")
        ...
        print(f"[PROGRESS] LightGBM training n_jobs: {self.lgbm_train_n_jobs}")
    ...
    def _validate_runtime_config(self) -> None:
        ...
        if self.train_chunk_size <= 0:
            raise ValueError("LGBM train_chunk_size must be positive.")

        if self.prediction_chunk_size <= 0:
            raise ValueError("LGBM prediction_chunk_size must be positive.")

        if self.lgbm_train_n_jobs == 0:
            raise ValueError("LGBM_TRAIN_N_JOBS cannot be 0.")
    ...
    def prepare_columns(self) -> Tuple[List[str], List[str], List[str]]:
        ...
        Returns:
            Tuple of merge columns, feature columns, and target columns.
        """
        print("[PROGRESS] Preparing LightGBM columns")
        ...
        if leakage_features:
            raise ValueError(f"Leakage-risk LightGBM input columns found: {leakage_features}")
        ...
        if engineered_targets:
            raise ValueError(
                "Engineered target columns found. LightGBM target must be raw X_s only: "
                f"{engineered_targets}"
            )

        print(f"[PROGRESS] LightGBM feature columns count: {len(feature_columns)}")
        print(f"[PROGRESS] LightGBM target columns count: {len(target_columns)}")
        print(f"[PROGRESS] LightGBM feature columns: {feature_columns}")
        print(f"[PROGRESS] LightGBM target columns: {target_columns}")
    ...
    def count_rows(self, merge_columns: List[str]) -> Dict[str, int]:
        ...
        Returns:
            Count dictionary.
        """
        print("[PROGRESS] Counting rows for LightGBM training")
        ...
        if counts["dev"] <= 0:
            raise ValueError("No dev rows found for LightGBM training.")

        print(f"[PROGRESS] Final LightGBM row counts: {counts}")
    ...
    def build_full_dev_memmaps(
        ...
        Args:
            merge_columns: Key columns.
            feature_columns: LightGBM input columns.
            target_columns: Raw X_s target columns.
            expected_dev_rows: Number of dev rows expected.

        Returns:
            Tuple of X_dev memmap and y_dev memmap.
        """
        print("[PROGRESS] Building full-dev LightGBM memmaps")

        memmap_paths = self._get_memmap_paths()

        if self.rebuild_memmap and self.memmap_dir.exists():
            print("[PROGRESS] Removing old LightGBM memmap cache directory")
        ...
        for scaled_chunk, context_chunk in zip(scaled_iter, context_iter):
            ...
            print("=" * 100)
            print(f"[PROGRESS] LightGBM memmap chunk #{chunk_index}")
            ...
            if x_nan_count > 0:
                raise ValueError(
                    f"LightGBM training features contain NaN in chunk {chunk_index}: "
                    f"{x_nan_count}"
                )

            if y_nan_count > 0:
                raise ValueError(
                    f"LightGBM training targets contain NaN in chunk {chunk_index}: "
                    f"{y_nan_count}"
                )
            ...
            if end_index > expected_dev_rows:
                raise ValueError(
                    "LightGBM memmap write exceeded expected dev rows. "
        ...
        if write_index != expected_dev_rows:
            raise ValueError(
                "LightGBM memmap written row count mismatch. "
        ...
        print("[PROGRESS] LightGBM full-dev memmaps built successfully")
    ...
    def train(
        ...
        """
        Train LightGBM Regressor on full dev memmap.

        Args:
            x_dev: Dev feature memmap.
            y_dev: Dev target memmap.
            feature_columns: Feature names.
            target_columns: Target names.
            row_counts: Row count dictionary.

        Returns:
            Model payload dictionary.
        """
        print("[TRAINING] LightGBM full-dev training started")
        print("[TRAINING] Model: LGBMRegressor wrapped by MultiOutputRegressor")
        print("[TRAINING] Library: LightGBM")
        ...
        try:
            if LGBMRegressor is None:
                raise ImportError("lightgbm is not installed. Install lightgbm.")

            started = perf_counter()

            lgbm_params = dict(Config.LGBM_PARAMS)
            lgbm_params["n_jobs"] = self.lgbm_train_n_jobs

            if "verbosity" not in lgbm_params:
                lgbm_params["verbosity"] = int(getattr(Config, "LGBM_VERBOSITY", -1))

            print(f"[TRAINING] LightGBM params used: {lgbm_params}")

            base_model = LGBMRegressor(**lgbm_params)

            # Keep n_jobs=1 here for memory safety.
            # The inner LightGBM model uses LGBM_TRAIN_N_JOBS.
            ...
            print(f"[TRAINING] LightGBM fit duration seconds: {fit_duration:.2f}")
            print(f"[TRAINING] LightGBM fit duration minutes: {fit_duration / 60.0:.2f}")

            trained_output_models = int(len(model.estimators_))
            print(f"[TRAINING] Number of output LightGBM models trained: {trained_output_models}")

            total_duration = perf_counter() - started

            payload: Dict[str, object] = {
                "model": model,
                "model_name": "lightgbm_digital_twin_regressor",
                "model_type": "LGBMRegressor_MultiOutputRegressor",
                "library": "LightGBM",
                "feature_columns": feature_columns,
                "target_columns": target_columns,
                "prediction_prefix": "lgbm_predicted_",
            ...
            print(f"[TRAINING] Saving LightGBM model payload safely to: {self.model_path}")
            ...
            print("[TRAINING] LightGBM model payload saved successfully")
            print(f"[TRAINING] LightGBM metadata saved to: {self.metadata_path}")
            print(f"[TRAINING] Total LightGBM training duration seconds: {total_duration:.2f}")
            print(f"[TRAINING] Total LightGBM training duration minutes: {total_duration / 60.0:.2f}")

            logger.info(
                "LightGBM full-dev trained. rows=%s targets=%s output_models=%s",
                row_counts["dev"],
                len(target_columns),
            ...
            print(f"[ERROR] LightGBM training failed: {exc}")
            logger.exception("LightGBM training failed.")
            raise RuntimeError("LightGBM training failed.") from exc

    def predict(self, expected_total_rows: int) -> int:
        """
        Predict raw X_s values for all rows.

        This version avoids this warning:

            X does not have valid feature names,
            but LGBMRegressor was fitted with feature names

        It does this by:
        - using a pandas DataFrame for prediction if the saved LightGBM model
          was fitted with feature names
        - using NumPy only when the saved model was fitted without feature names
        ...
        """
        print("[PROGRESS] LightGBM streaming prediction started")
        ...
        try:
            ...
            prediction_prefix = str(payload.get("prediction_prefix", "lgbm_predicted_"))
            ...
            temp_output_path = output_path.with_suffix(
                output_path.suffix + f".{self.run_id}.tmp"
            )
            ...
            print(f"[PROGRESS] Final LightGBM predictions CSV path: {output_path}")
            print(f"[PROGRESS] Temporary LightGBM predictions CSV path: {temp_output_path}")
            ...
            # Detect whether the saved inner LightGBM estimators were fitted
            # with pandas feature names.
            first_estimator = None

            if hasattr(model, "estimators_") and len(model.estimators_) > 0:
                first_estimator = model.estimators_[0]

            was_fitted_with_feature_names = bool(
                first_estimator is not None
                and hasattr(first_estimator, "feature_names_in_")
            )

            print(
                "[PROGRESS] LightGBM fitted with feature names: "
                f"{was_fitted_with_feature_names}"
            )
            ...
            for scaled_chunk, context_chunk in zip(scaled_iter, context_iter):
                ...
                print("=" * 100)
                print(f"[PROGRESS] LightGBM prediction chunk #{chunk_index}")
                ...
                if was_fitted_with_feature_names:
                    predictions = model.predict(x_batch_df).astype(np.float32, copy=False)
                else:
                    x_batch_np = x_batch_df.to_numpy(dtype=np.float32, copy=False)
                    predictions = model.predict(x_batch_np).astype(np.float32, copy=False)
                    del x_batch_np
                ...
                if predictions.shape[1] != len(target_columns):
                    raise ValueError(
                        "LightGBM prediction target count mismatch. "
                ...
                print(f"[PROGRESS] Total LightGBM prediction rows written: {total_rows_written}")
            ...
            if total_rows_written != expected_total_rows:
                raise ValueError(
                    "LightGBM prediction row count mismatch. "
            ...
            print("[PROGRESS] LightGBM predictions CSV written successfully")
            print(f"[PROGRESS] LightGBM prediction records written: {total_rows_written}")
            print(f"[PROGRESS] LightGBM prediction duration seconds: {duration:.2f}")
            print(f"[PROGRESS] LightGBM prediction duration minutes: {duration / 60.0:.2f}")

            logger.info("LightGBM prediction completed. rows=%s", total_rows_written)

            return total_rows_written

        except Exception as exc:
            print(f"[ERROR] LightGBM prediction failed: {exc}")
            logger.exception("LightGBM prediction failed.")
            raise RuntimeError("LightGBM prediction failed.") from exc

    def run(self) -> Dict[str, object]:
        """
        Run LightGBM full-dev training and streaming prediction.

        Returns:
            Stage response dictionary.
        """
        print("[PROGRESS] LightGBMTwin.run started")
        ...
        try:
            ...
            print("[PROGRESS] Releasing LightGBM training memmap references before prediction")
            del x_dev
            del y_dev
            gc.collect()

            if self.cleanup_memmap_after_training and self.memmap_dir.exists():
                print("[PROGRESS] Cleaning up LightGBM memmap directory after successful training")
                shutil.rmtree(self.memmap_dir)
            ...
            response = {
                "status": "success",
                "message": (
                    "LightGBM Digital Twin trained on full dev split using "
            ...
            print(f"[PROGRESS] LightGBM twin response: {response}")
            return response

        except Exception as exc:
            print(f"[ERROR] LightGBM twin stage failed: {exc}")
            logger.exception("LightGBM twin stage failed.")
            raise RuntimeError("LightGBM twin stage failed.") from exc


def run_lightgbm_twin() -> Dict[str, object]:
    """
    Execute LightGBM Digital Twin stage.

    Returns:
        Stage response dictionary.
    """
    service = LightGBMTwin()
    return service.run()


if __name__ == "__main__":
    print("[PROGRESS] lightgbm_twin.py execution started")
    result = run_lightgbm_twin()
    print("[PROGRESS] lightgbm_twin.py execution finished successfully")
    print(result)